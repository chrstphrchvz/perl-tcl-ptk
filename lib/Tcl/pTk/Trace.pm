...
#
# ->[0] = a reference to the variable's Tcl::pTk::Tie::Watch object
...
my %OP = (			# trace to Tcl::pTk::Tie::Watch operation map
    r => '-fetch',
    w => '-store',
    u => '-destroy',
);
...
sub fetch {
    ...
    my $self = shift;                          # Tcl::pTk::Tie::Watch object
...
sub Tcl::pTk::Widget::traceVariable {
    ...
    if ( not defined $trace ) {
        my $watch = Tcl::pTk::Tie::Watch->new(
            -variable => $vref,
            -fetch    => [ \&fetch,   $vref ],
            -store    => [ \&store,   $vref ],
...
sub Tcl::pTk::Widget::traceVdelete {
...
Tcl::pTk::Trace - emulate Tcl/Tk B<trace> functions.
...
=head1 HISTORY
 ...
 Updated for use with Tcl::pTk 2008/10/14
