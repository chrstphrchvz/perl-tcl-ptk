   ...
   if (ref $widget)
    {
     ...
     if ($ref eq 'ARRAY')
      {
       $widget = Tcl::pTk::Configure->new(@$widget);
       ...
       foreach my $key (%$widget)
        {
         foreach my $sw (_makelist($widget->{$key}))
          {
           push(@subwidget,Tcl::pTk::Configure->new($sw,$key));
          }
        }
      }
     else
    ...
    {
     my ($method) = ($opt =~ /^-?(.*)$/);
     push(@subwidget,Tcl::pTk::Configure->new($method,$method,$cw))
    }
   elsif ($widget eq 'SETMETHOD')
    {
     my ($method) = ($opt =~ /^-?(.*)$/);
     push(@subwidget,Tcl::pTk::Configure->new($method,'_cget',$cw,@arg))
    }
   elsif ($widget eq 'SELF')
    {
     push(@subwidget,Tcl::pTk::Configure->new('Tcl::pTk::Widget::configure', 'Tcl::pTk::Widget::cget', $cw,@arg))
    }
   elsif ($widget eq 'PASSIVE')
    {
     push(@subwidget,Tcl::pTk::Configure->new('_configure','_cget',$cw,@arg))
    }
   elsif ($widget eq 'CALLBACK')
    {
     push(@subwidget,Tcl::pTk::Configure->new('_callback','_cget',$cw,@arg))
    }
   else
    {
...
sub _callback
{
 my ($cw,$opt,$val) = @_;
 $cw->BackTrace('Wrong number of args to configure') unless (@_ == 3);
 $val = Tcl::pTk::Callback->new($val) if defined($val) && ref($val);
 $cw->{Configure}{$opt} = $val;
}
