...
sub Populate {
    ...
    $top->configure(-relief => 'raised', -bd => 1) unless $Tcl::pTk::platform eq 'MSWin32';
    my $bot = $cw->Component('Frame', 'bottom');
    $bot->configure(-relief => 'raised', -bd => 1) unless $Tcl::pTk::platform eq 'MSWin32';
	...
	$cw->Advertise("B_$bl" => $b);
        if ($Tcl::pTk::platform eq 'MSWin32')
         {
