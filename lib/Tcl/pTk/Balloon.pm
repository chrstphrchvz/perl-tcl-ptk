...
sub Populate {
    ...
    $w->ConfigSpecs(-installcolormap => ['PASSIVE', 'installColormap', 'InstallColormap', 0],
		    ...
		    -borderwidth => ['SELF', 'borderWidth', 'BorderWidth', 1],
                    ...
                    -arrow=>['PASSIVE', 'arrow','arrow',0],  # Flag = 1 to display an arrow in the balloon (Changed in Tcl::pTk to be off)
		   );
}
...
sub attach {
    ...
    foreach my $key (grep(/command$/,keys %args))
     {
      $args{$key} = Tcl::pTk::Callback->new($args{$key});
     }
...
sub SwitchToClient {
    my ($w, $client) = @_;
    return unless Tcl::pTk::Exists($w);
    return unless Tcl::pTk::Exists($client);
...
sub SetStatus {
    ...
    if (defined $s and Tcl::pTk::Exists($s)) {
