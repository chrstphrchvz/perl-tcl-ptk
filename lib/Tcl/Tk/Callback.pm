...
Tcl::Tk::Callback -  Callback Helper class for Tcl::Tk


=head1 SYNOPSIS

        # Create Callback - No Arg Subref form
	$cb = Tcl::Tk::Callback->new(\&lots);
        
        # Create Callback - Subref form, with args
	$cb = Tcl::Tk::Callback->new([\&lots, $arg1, $arg2]);
...
I<Tcl::Tk::Callback> is a helper class, modeled after I<Tk::Callback> (documented in L<Tk::callbacks> ), that 
provides methods for constructing and executing callbacks for the L<Tcl::Tk> package. 
...
Hash ref showing mapping of any I<Tcl::Tk::Ev> objects in the args to their position in the stored callback.
...
Constructor: Creates a new Tcl::Tk::Callback instance. 
...
sub new{
        ...
        # Examine args and determine if any are Tcl::Tk::Ev objects, which need event info substitution performed
        ...
        if ($#callback >= $argStart){ # args present
                foreach my $argIndex($argStart..$#callback){
                        ...
                        if( ref($arg) and ref($arg) eq 'Tcl::Tk::Ev'){
                                my $subst = $arg->[0];
...
execute this L<Tcl::Tk::Callback> when the bind event occurs. Handles any Event pattern substitution thru
