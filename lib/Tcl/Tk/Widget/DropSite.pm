package Tcl::Tk::Widget::DropSite;
require Tcl::Tk::Widget::DragDrop::Common;
require Tcl::Tk::Widget::DragDrop::Rect;


use base  qw(Tcl::Tk::Widget::DragDrop::Common Tcl::Tk::Widget::DragDrop::Rect);

Construct Tcl::Tk::Widget 'DropSite';
...
#Tcl::Tk::Widget::DragDrop->Tcl::Tk::Widget::DragDrop::Common::Type('Local');
...
sub NoteSites
{
 ...
 unless (grep($_ == $t,@toplevels))
  {
   $Tcl::Tk::Widget::DragDrop::types{'Local'} = $class if (@$sites);
   push(@toplevels,$t);
...
sub new
{
 ...
 foreach $type (@{$args{'-droptypes'}})
  {
   Tcl::Tk::Widget::DropSite->import($type) unless (exists $type{$type});
   my $class = $type{$type};
   $class->InitSite($obj);
 ...
 $w->Tcl::Tk::Widget::bind($obj,'<Map>',[$obj,'DropSiteUpdate']);
 $w->Tcl::Tk::Widget::bind($obj,'<Unmap>',[$obj,'DropSiteUpdate']);
 $w->Tcl::Tk::Widget::bind($obj,'<Configure>',[$obj,'DropSiteUpdate']);
 $t->Tcl::Tk::Widget::bind($class,'<Configure>',[\&TopSiteUpdate,$t]);
