...
Tcl::Tk::Widget::TableMatrix::Spreadsheet - Table Display with Spreadsheet-like bindings.
...
L<Tcl::Tk::Widget::TableMatrix::Spreadsheet> is a L<Tcl::Tk::Widget::TableMatrix>-derived widget that implements
...
or the F2 button is pressed. The default L<Tcl::Tk::Widget::TableMatrix> behavior is for the
...
Widget methods, options, etc, are inherited from the L<Tcl::Tk::Widget::TableMatrix> widget. See its 
...
package Tcl::Tk::Widget::TableMatrix::Spreadsheet;
...
use base qw/ Tcl::Tk::Derived Tcl::Tk::Widget::TableMatrix/;

$VERSION = '1.23';


Tcl::Tk::Widget->Construct("Spreadsheet");
		  ...
		  [sub
		   {
		    my $w = shift;
		    my ($x, $y) = @_;
		    $w->Paste($w->index('@' . $x.",".$y),'PRIMARY') unless ($Tcl::Tk::Widget::TableMatrix::tkPriv{'mouseMoved'});
		   }, Ev('x'), Ev('y')
                   ]
		 );
...
sub MoveCell{
	...
	if ($w->cget('-selectmode') eq 'browse')
	...
	elsif ($w->cget('-selectmode') eq 'extended')
	 {
	  ...
	  $Tcl::Tk::Widget::TableMatrix::tkPriv{'tablePrev'} = $cell;
	 }
}	
...
sub B1Motion {
	...
	my $rowColResize = $w->{rowColResize};  # Flag = 1 if cursor has been changed for a row/col resize
        ...
        if( $rowColResize || !$Tcl::Tk::Widget::TableMatrix::tkPriv{borderInfo}  ) {
