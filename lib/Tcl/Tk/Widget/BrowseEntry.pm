...
#
#
# Converted to Tcl::Tk by John Cerney

package Tcl::Tk::Widget::BrowseEntry;
...
require Tcl::Tk::Widget::LabEntry;
...
sub Populate {
    ...
    $w->{_BE_Style} = delete $args->{-style} || $Tcl::Tk::platform;
...
#      }
    ...
    my $b = $w->$Button(-bitmap => '@' . Tcl::Tk->findINC($w->{_BE_Style} eq 'MSWin32' ? 'arrowdownwin.xbm' : 'cbxarrow.xbm'));
    $w->Advertise('entry' => $e);
    $w->Advertise('arrow' => $b);
    ...
    if ($w->{_BE_Style} eq 'MSWin32' and $Tcl::Tk::platform eq 'MSWin32') {
...
sub SetBindings {
    ...
    # set bind tags
    $w->bindtags([$w, 'Tcl::Tk::BrowseEntry', $w->toplevel, 'all']);
...
sub PopupChoices {
	...
	my $rootheight;
	if ($Tcl::Tk::platform eq 'MSWin32' and $^O eq 'MSWin32') {
	    eval {
...
sub Popdown {
    my ($w) = @_;
    if ($w->{'_BE_savefocus'} && Tcl::Tk::Exists($w->{'_BE_savefocus'})) {
...
sub ButtonHack {
    ...
    my $b = $w->Subwidget('arrow');
    if ($w->{'_BE_buttonHack'}) {
	#$b->butUp; # This function not available for Tcl::Tk
    }
}
...
sub _set_edit_state {
	...
	if (exists $w->{_BE_OriginalCursor}) {
        ...
        # Button1 method of Entry isn't there in Tcl::Tk Is it really needed?
	#if ($w->{_BE_Style} eq 'MSWin32') { 
