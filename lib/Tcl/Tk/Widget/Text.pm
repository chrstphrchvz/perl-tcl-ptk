...
use base  qw(Tcl::Tk::Widget::Clipboard Tcl::Tk::Widget);
...
sub ClassInit
{
 ...
 $mw->bind($class, '<3>', ['PostPopupMenu', Tcl::Tk::Ev('X'), Tcl::Tk::Ev('Y')]  ); # right-click menu
 ...
 $class = 'Text';
 $mw->bind($class,'<KeyPress>',['InsertKeypress',Tcl::Tk::Ev('A')]);
...
sub selectLine
{
 ...
 Tcl::Tk::catch { $w->markSet('insert','sel.first') };
}
...
{
 my ($w) = @_;
 my $sel = Tcl::Tk::catch { $w->tag('nextrange','sel','1.0','end') };
 if (defined $sel)
  {
...
{
 my ($w) = @_;
 Tcl::Tk::catch { $w->Insert($w->SelectionGet) }
}
