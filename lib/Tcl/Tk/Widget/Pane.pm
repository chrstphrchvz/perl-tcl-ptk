...
#   when we set the Tcl::Tk::Widget::Pane::ISA below
#
package Tcl::Tk::Widget::ScrollableFrame;

package Tcl::Tk::Widget::Pane;
...
#use base qw(Tcl::Tk::Derived Tcl::Tk::Widget::ScrollableFrame);
@Tcl::Tk::Widget::Pane::ISA = (qw/ Tcl::Tk::Derived Tcl::Tk::Widget::ScrollableFrame /);

Tcl::Tk::Widget->Construct('Pane');
...
sub Populate {
    ...
    $frame = $cw->interp->declare_widget($frame, 'Tcl::Tk::Widget::Frame'); # turn path name into widget
...
# Manager wrapper method supplied here so that $cw->manager calls in Default method of Tcl::Tk::Frame.pm
#  Doesnt't get delegated to the frame
sub manager{
        my $self = shift;
        $self->Tcl::Tk::Widget::manager(@_);
}
...
sub see{
      ...
      unless( @newArgs <= 2){
              croak("Invalid args supplied to Tcl::Tk::Pane::see args =  ",join(", ", @args));
      }
      ...
      if( $anchor ){
              ...
              unless( defined( $anchorTrans ) ){
                      croak("Invalid -anchor arg $anchor supplied to Tcl::Tk::Pane::see");
              }
...
Tcl::Tk::Widget::Pane - A window panner
...
B<Tcl::Tk::Widget::Pane> provides a scrollable frame widget. Once created it can be
treated as a frame, except it is scrollable.
        
B<Note:> This L<Tcl::Tk> Implementation of the L<Tk::Pane> widget. It uses the Tcl/Tk I<BWidget> package
