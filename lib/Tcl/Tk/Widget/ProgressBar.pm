package Tcl::Tk::Widget::ProgressBar;
...
use base qw(Tcl::Tk::Derived Tcl::Tk::Widget::Canvas);

Construct Tcl::Tk::Widget 'ProgressBar';
...
sub _arrange {
	...
	($w,$h) = ($h,$w) if $horz;
 	...
 	#$c->GeometryRequest($w,$h);
        # Tcl::Tk doesn't support GeometryRequest on non-toplevel windows, so we directly set the
        #   width/height of the parent canvas
        $c->Tcl::Tk::Widget::Canvas::configure(-width => $w, -height => $h);
	$c->parent->update;
	$c->update;
...
Tcl::Tk::Widget::ProgressBar - A graphical progress bar
...
B<Tcl::Tk::Widget::ProgressBar> provides a widget which will show a graphical representation
