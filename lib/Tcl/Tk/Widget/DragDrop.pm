package Tcl::Tk::Widget::DragDrop;
require Tcl::Tk::Widget::DragDrop::Common;
require Tcl::Tk::Widget::Toplevel;


use base  qw(Tcl::Tk::Widget::DragDrop::Common Tcl::Tk::Widget::Toplevel);


my $useWmRelease = 0; # We dont' ever use wmRelease

Construct Tcl::Tk::Widget 'DragDrop';
...
sub ClassInit
{
 ...
 $mw->bind($class,'<Any-ButtonRelease>',['Drop', Tcl::Tk::Ev('X'), Tcl::Tk::Ev('Y') ]);
 $mw->bind($class,'<Any-Motion>',['Drag', Tcl::Tk::Ev('X'), Tcl::Tk::Ev('Y')]);
 return $class;
}
   ...
   if (defined $val)
    {
     ...
     foreach $type (@$val)
      {
       Tcl::Tk::Widget::DragDrop->import($type);
      }
    }
   $$var = $val;
  }
...
sub event
{
 my ($w,$opt,$value) = @_;
 # delete old bindings
 $w->parent->Tcl::Tk::Widget::bind($value,[$w,'StartDrag', Tcl::Tk::Ev('X'), Tcl::Tk::Ev('Y')]);
}
...
sub Drop
{
 ...
 Tcl::Tk::catch { $token->grabRelease };
 if (defined $site)
  {
