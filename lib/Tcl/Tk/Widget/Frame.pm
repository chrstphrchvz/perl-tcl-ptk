#
# Dummy Frame.pm
#  Needed so 'use base('Tcl::Tk::Widget::Frame') works in megawidgets (e.g. SlideSwitch.pm)

package Tcl::Tk::Widget::Frame;
 ...
 foreach $sc ($sw->configure)
  {
   ...
   unless ($skip{$option})
    {
     $option =~ s/^-(.*)/-$name\u$1/;
     $info[0] = Tcl::Tk::Configure->new($sw,$info[0]);
     pop(@info);
 ...
 if (@_ > 1)
  {
   if (defined($w) && !defined($val))
    ...
    {
     # Using Direct call to Tcl::Tk::Label rather than a plain 'Label' to avoid delegation
     ...
     $w = $cw->Tcl::Tk::Label(-textvariable => $cw->labelVariable);
