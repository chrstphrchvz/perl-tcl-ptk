...
#  Modified for use in Tcl::Tk
package Tcl::Tk::Widget::Balloon;


use Tcl::Tk;
use Carp;
use Tcl::Tk::Widget::Toplevel;

Tcl::Tk::Widget->Construct('Balloon');
use base qw(Tcl::Tk::Widget::Toplevel);
...
sub ClassInit {
    my ($class, $mw) = @_;
    $mw->bind('all', '<Motion>', ['Tcl::Tk::Widget::Balloon::Motion', Tcl::Tk::Ev('X'), Tcl::Tk::Ev('Y'), Tcl::Tk::Ev('s')]);
    $mw->bind('all', '<Leave>',  ['Tcl::Tk::Widget::Balloon::Motion', Tcl::Tk::Ev('X'), Tcl::Tk::Ev('Y'), Tcl::Tk::Ev('s')]);
    $mw->bind('all', '<Button>', 'Tcl::Tk::Widget::Balloon::ButtonDown');
    $mw->bind('all', '<ButtonRelease>', 'Tcl::Tk::Widget::Balloon::ButtonUp');
    return $class;
}
...
sub attach {
    ...
    foreach my $key (grep(/command$/,keys %args))
     {
      $args{$key} = Tcl::Tk::Callback->new($args{$key});
     }
...
sub SwitchToClient {
    my ($w, $client) = @_;
    return unless Tcl::Tk::Exists($w);
    return unless Tcl::Tk::Exists($client);
...
sub grabBad {
    ...
    return 0 unless Tcl::Tk::Exists($client);
    my $g = $w->grabCurrent;
    return 0 unless defined $g;
    return 0 if $g->isa('Tcl::Tk::Widget::Menu');
...
sub SetStatus {
    ...
    if (defined $s and Tcl::Tk::Exists($s)) {
