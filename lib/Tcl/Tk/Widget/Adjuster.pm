package Tcl::Tk::Widget::Adjuster;

##### Implementation of Tk::Adjuster in Tcl::Tk ###########
## This code is incomplete for Tcl::Tk (because GeometryRequest method isn't supported)
...
Construct Tcl::Tk::Widget qw(Adjuster);

{package Tcl::Tk::Widget::Adjuster::Item;
...
sub Mapped
{
 ...
 if ($m =~ /^(?:pack|grid)$/)
  {
   my $infoMethod = $m."Info";
   my $propMethod = $m."Propagate";
   my %info = $w->$infoMethod;
   my $master = $info{'-in'};
   
   # Map the pathname returned to an actual widget 
   unless( ref($master) ){
           my $int = $w->interp;
           if ($master eq '.') {
                $master =  $int->mainwindow;
           }
           else{
                $master = $int->widget($res);
           }
   }
...
sub Populate
{
 ...
 $w->{'sep'} = Tcl::Tk::Widget::Adjuster::Item->new($w,-bd => 1, -relief => 'sunken');
 $w->{'but'} = Tcl::Tk::Widget::Adjuster::Item->new($w,-bd => 1, -width => 8, -height => 8, -relief => 'raised');
 ...
 my $cs = $w->ConfigSpecs(-widget => ['PASSIVE','widget','Widget',$w->Parent],
                 ...
                 -foreground => [Tcl::Tk::Configure->new($w->{'lin'},'-background'),'foreground','Foreground','black'],
