...
#
# ->[0] = a reference to the variable's Tcl::Tk::Tie::Watch object
...
my %TCL_Traced;                 # Hash of variable refs that were already
                                # being tied by the Tcl package when traceVariable
                                #  was called. (We keep track of here and call
                                #    the Tcl::Var store and fetch routines to keep
                                #    the connection between tcl and perl.)
                                
my %OP = (			# trace to Tcl::Tk::Tie::Watch operation map
    r => '-fetch',
    w => '-store',
    u => '-destroy',
);
...
sub fetch {
    ...
    my $self = shift;                          # Tcl::Tk::Tie::Watch object
    my $aref = $self->Args('-fetch');          # argument reference
    
    my $vref = $aref->[0];

    my $val;
    # Call the Tcl::Var FETCH, if one was there when the trace was created
    if( defined(my $tiedObj = $TCL_Traced{$vref})){
            $val = $tiedObj->FETCH();
    }
    else{  # Otherwise, just do the normal fetch
            $val = $self->Fetch(@_);               # get variable's current value
    }
...
sub store {
    ...
    my $vref = $aref->[0];
    # Call the Tcl::Var STORE, if one was there when the trace was created
    if( defined(my $tiedObj = $TCL_Traced{$vref})){
            $tiedObj->STORE($self->Fetch());
    }
...
sub Tcl::Tk::Widget::traceVariable {
    ...
    my $tiedObj;
    if( defined($tiedObj = tied($$vref)) and ref($tiedObj) eq 'Tcl::Var' ){
            $TCL_Traced{$vref} = $tiedObj;
    }
...
sub Tcl::Tk::Widget::traceVdelete {
    ...
    if ( defined $TRACE{$vref}->[0] ) {
        ...
        # If variable was traced by Tcl, put the trace back
        if( defined( $TCL_Traced{$vref})){
                my $interp = $parent->interp;
                my $nm = "::perl::$vref"; # use same naming convention Tcl.pm does
                tie $$vref, 'Tcl::Var', $interp, $nm;
                delete $TCL_Traced{$vref};
        }
    }

} # end traceVdelete

sub Tcl::Tk::Widget::traceVinfo {
...
Tcl::Tk::Trace - emulate Tcl/Tk B<trace> functions.
...
=head1 HISTORY
 ...
 Updated for use with Tcl::Tk 2008/10/14
