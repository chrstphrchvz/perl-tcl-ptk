#
# Megawidget support for Tcl::Tk

package Tcl::Tk::Derived;
...
sub Subconfigure
{
 ...
 foreach $widget (_makelist($widget))
  {
   # Check to see if the option widget is the same as the top-level widget
   $widget = 'SELF' if (ref($widget) && (ref($widget) eq ref($cw))
           && ($widget->path eq $cw->path) );
   if (ref $widget)
    {
     my $ref = ref $widget;
     if ($ref eq 'ARRAY')
      {
       $widget = Tcl::Tk::Configure->new(@$widget);
       ...
       foreach my $key (%$widget)
        {
         foreach my $sw (_makelist($widget->{$key}))
          {
           push(@subwidget,Tcl::Tk::Configure->new($sw,$key));
          }
        }
      }
     else
    ...
    {
     my ($method) = ($opt =~ /^-?(.*)$/);
     push(@subwidget,Tcl::Tk::Configure->new($method,$method,$cw))
    }
   elsif ($widget eq 'SETMETHOD')
    {
     my ($method) = ($opt =~ /^-?(.*)$/);
     push(@subwidget,Tcl::Tk::Configure->new($method,'_cget',$cw,@arg))
    }
   elsif ($widget eq 'SELF')
    {
     push(@subwidget,Tcl::Tk::Configure->new('Tcl::Tk::Widget::configure', 'Tcl::Tk::Widget::cget', $cw,@arg))
    }
   elsif ($widget eq 'PASSIVE')
    {
     push(@subwidget,Tcl::Tk::Configure->new('_configure','_cget',$cw,@arg))
    }
   elsif ($widget eq 'CALLBACK')
    {
     push(@subwidget,Tcl::Tk::Configure->new('_callback','_cget',$cw,@arg))
    }
   else
    {
...
sub _callback
{
 my ($cw,$opt,$val) = @_;
 $cw->BackTrace('Wrong number of args to configure') unless (@_ == 3);
 $val = Tcl::Tk::Callback->new($val) if defined($val) && ref($val);
 $cw->{Configure}{$opt} = $val;
}
...
##################################################################3
# ConfigDefault:
#   Populate the supplied $args hash-ref with default values, based
#   on the widgets ConfigSpecs
sub ConfigDefault
{
 my ($cw,$args) = @_;
 ...
 unless (exists($specs->{'-background'}))
  {            
  Tcl::Tk::catch { $cw->Tcl::Tk::Widget::cget('-background') };
   my (@bg) = $@ ? ('PASSIVE') : ('SELF');
           ...
           if (defined $info->[1] && defined $info->[2])
            {
             ...
             $info->[3] = $db if (defined $db and $db ne '');
            }
           $args->{$opt} = $info->[3];
          }
        }
