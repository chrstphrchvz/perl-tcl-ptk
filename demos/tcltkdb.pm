...
BEGIN{$DB::sigint_disable='bad gguess... but...'}
{$DB::sigint_disable='bad gguess... but...'}
package DB ;
...
#TODO  require Tk::Dialog;
#TODO  require Tk::TextUndo ;
#TODO  require Tk::ROText;
#TODO  require Tk::NoteBook ;
#TODO  require Tk::HList ;
#TODO  require Tk::Table ;
...
sub BEGIN {
 ...
 $Devel::tcltkdb::eval_dump_indent = $ENV{'PTKDB_EVAL_DUMP_INDENT'} || 1 ;
   ...
   @Devel::tcltkdb::scrollbar_cfg = ('-scrollbars' => 'se') ;
   }
   else {
   ...
   #
 $Devel::tcltkdb::expr_depth = -1 ;
 $Devel::tcltkdb::add_expr_depth = 1 ; # how much further to expand an expression when clicked

 $Devel::tcltkdb::linenumber_format = $ENV{'PTKDB_LINENUMBER_FORMAT'} || "%05d " ;
 $Devel::tcltkdb::linenumber_length = 5 ;

 $Devel::tcltkdb::linenumber_offset = length sprintf($Devel::tcltkdb::linenumber_format, 0) ;
 $Devel::tcltkdb::linenumber_offset -= 1 ;
   ...
   #
 $Devel::tcltkdb::DataDumperAvailable = 1 ; # assuming that it is now
 $Devel::tcltkdb::useDataDumperForEval = $Devel::tcltkdb::DataDumperAvailable ;
...
#
sub add_exprs {
  push @{$DB::window->{'expr_list'}}, map { 'expr' => $_, 'depth' => $Devel::tcltkdb::expr_depth }, @_ ;
} # end of add_exprs
...
sub DoQuit {
  ...
  $self->save_bookmarks($self->{BookMarksPath}) if $Devel::tcltkdb::DataDumperAvailable && $self->{'bookmarks_changed'};
...
sub setup_menu_bar {
  ...
  #
  my @dataDumperEnableOpt = ( state => 'disabled' ) unless $Devel::tcltkdb::DataDumperAvailable ;
  ...
  $items = [ [ 'command' => 'Enter Expression', -accelerator => 'Alt+E', -command => sub { $self->EnterExpr() } ],
             ...
             [ 'checkbutton' => "Use DataDumper for Eval Window?", -variable => \$Devel::tcltkdb::useDataDumperForEval, @dataDumperEnableOpt ]
              ] ;
  ...
  $self->{stepin_button} = $self->{button_bar}->Button(-text, => "Step In", @Devel::tcltkdb::button_font,
                                                       -command => $stepInSub) ;
  $self->{stepin_button}->pack(-side => 'left') ;

  $self->{stepover_button} = $self->{button_bar}->Button(-text, => "Step Over", @Devel::tcltkdb::button_font,
                                                         -command => $stepOverSub) ;
  $self->{stepover_button}->pack(-side => 'left') ;

  $self->{return_button} = $self->{button_bar}->Button(-text, => "Return", @Devel::tcltkdb::button_font,
                                                       -command => $returnSub) ;
  $self->{return_button}->pack(-side => 'left') ;

  $self->{run_button} = $self->{button_bar}->Button(-background => 'green', -text, => "Run", @Devel::tcltkdb::button_font,
                                                    -command => $runSub) ;
  $self->{run_button}->pack(-side => 'left') ;

  $self->{run_to_button} = $self->{button_bar}->Button(-text, => "Run To", @Devel::tcltkdb::button_font,
                                                       -command => $runToSub) ;
  $self->{run_to_button}->pack(-side => 'left') ;

  $self->{breakpt_button} = $self->{button_bar}->Button(-text, => "Break", @Devel::tcltkdb::button_font,
...
sub save_bookmarks {
  ...
  return unless $Devel::tcltkdb::DataDumperAvailable ; # we can't save without the data dumper
  local(*F) ;
...
sub expr_expand {
  ...
  $DB::window->{'expr_list'}->[$index]->{'depth'} += $Devel::tcltkdb::add_expr_depth ;
    #
...
sub setup_frames {
  ...
  $self->{data_list} = $self->{'data_page'}->Scrolled('HList',
                                                    @Devel::tcltkdb::scrollbar_cfg, 
                                                      separator => $Devel::tcltkdb::pathSep,
                                                    @Devel::tcltkdb::expression_text_font,
                                                      -command => \&Devel::tcltkdb::expr_expand,
                                                      -selectmode => 'multiple'
                                                      ) ;
...
sub configure_text {
  ...
  if ( $Devel::tcltkdb::DataDumperAvailable ) {
    $self->{'balloon_dumper'} = new Data::Dumper([$place_holder]) ;
    $self->{'balloon_dumper'}->Terse(1) ;
    $self->{'balloon_dumper'}->Indent($Devel::tcltkdb::eval_dump_indent) ;
...
sub simplePromptBox {
  ...
  $top->Button( -text => "Cancel", -command => sub { &$cancelSub() if $cancelSub ; $top->destroy() }, @Devel::tcltkdb::button_font,
                )->pack(-side => 'left', -fill => 'both', -expand => 1) ;
...
sub add_brkpt_to_brkpt_page { 
  ...
  $width = $frm->cget('-width') ;#TODO < Must be widget method
...
sub fixExprPath {
  ...
  for (@pathList) {
    s/$Devel::tcltkdb::pathSep/$Devel::tcltkdb::pathSepReplacement/go ;
  } # end of path list
  ...
  if( $type eq 'ARRAY' or "$theRef" =~ /ARRAY/ ) {
    ...
    foreach $r ( @{$theRef} ) {
      ...
      if( grep $_ == $r, @$reusedRefs ) { # check to make sure that we're not doing a single level self reference
        eval {
          $dl->add($dirPath .  fixExprPath($name) . $Devel::tcltkdb::pathSep . "__ptkdb_self_path" . $selfCnt++, -text => "[$idx] = $r REUSED ADDR") ;
        } ;
        $self->DoAlert($@) if( $@ ) ;
        next ;
      }
      ...
      push @$reusedRefs, $r ;
      $result = $self->insertExpr($reusedRefs, $dl, $r, "[$idx]", $depth-1, $dirPath . fixExprPath($name) . $Devel::tcltkdb::pathSep) unless $depth == 0 ;
      pop @$reusedRefs ;
...
#
  ...
  foreach $r ( @$theRef{@theKeys} ) { # slice out the values with the sorted list
    ...
    if( grep $_ == $r, @$reusedRefs ) { # check to make sure that we're not doing a single level self reference
      eval {
        $dl->add($dirPath .  fixExprPath($name) . $Devel::tcltkdb::pathSep . "__ptkdb_self_path" . $selfCnt++, -text => "$theKeys[$idx++] = $r REUSED ADDR") ;
      } ;
      print "bad path $@\n" if( $@ ) ;
      next ;
    }
    ...
    $result = $self->insertExpr($reusedRefs,                              # recursion protection
                                ...
                                $dirPath . $name . $Devel::tcltkdb::pathSep # path to add to
                                ) unless $depth == 0 ;
...
sub set_file {
  ...
  $text->insert('end', map {
    ...
    $lineStr = sprintf($Devel::tcltkdb::linenumber_format, $i++) . $_ ; # line number + text of the line
		...
		#VVV!!!substr $lineStr, -2, 1, '' if $isWin32 ; # removes the CR from win32 instances
...
sub retrieve_text_expr {
  ...
  $offset = $Devel::tcltkdb::linenumber_length + 1 ; # line number text + 1 space
...
sub Initialize {
 ...
 $DB::startupFname = $fName ;
  ...
  if( $ENV{'PTKDB_RESTART_STATE_FILE'} && $Devel::tcltkdb::DataDumperAvailable && -e $ENV{'PTKDB_RESTART_STATE_FILE'} ) {
    ##
    ...
    &DB::restoreState($fName) if $Devel::tcltkdb::DataDumperAvailable ;
  }
     ...
     $DB::window->{'quick_entry'}->delete(0, 'end') ; # clear old text
       ...
       $DB::window->{'quick_dumper'}->Values( [ $#result == 0 ? @result : \@result ] ) ;
         ...
         if( $evt eq 'expr' ) {
           ...
           if(  @result == 1 ) {
             $r = $DB::window->insertExpr([ $result[0] ], $DB::window->{'data_list'}, $result[0], $DB::window->{'expr'}, $Devel::tcltkdb::expr_depth) ;
           }
           else {
             $r = $DB::window->insertExpr([ \@result ], $DB::window->{'data_list'}, \@result, $DB::window->{'expr'}, $Devel::tcltkdb::expr_depth)  ;
           }
           ...
           # next time through.  
           #
           push @{$DB::window->{'expr_list'}}, { 'expr' => $DB::window->{'expr'}, 'depth' => $Devel::tcltkdb::expr_depth } if $r ;
